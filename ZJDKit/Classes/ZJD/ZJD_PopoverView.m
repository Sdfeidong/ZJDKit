...
@interface ZJD_PopoverView ()<UITableViewDataSource, UITableViewDelegate>
...
@implementation ZJD_PopoverView
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        // Initialization code
        self.borderColor = RGBACOLOR(200, 199, 204, 1);
...
-(CGRect)getViewFrame
{
    ...
    for (NSString *title in self.titleArray) {
        CGFloat width =  [ToolClass string:title boundingRectWithWidth:kMScreen_W font:TITLE_FONT].width;
    ...
    if ((frame.origin.x + frame.size.width) > 315) {
        frame.origin.x = kMScreen_W - 2 - frame.size.width;
    }
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.textLabel.textColor = Color_TextWhite_ff;
    
    UIView *line = [[UIView alloc] initWithFrame:CGRectMake(0, 44, kMScreen_W, 0.5)];
    line.backgroundColor = UIColorFromHexadecimal(0x1877d9);
