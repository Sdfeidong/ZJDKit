...
- (UIButton *)skipBtn{
    ...
    if(_skipBtn == nil){
        ...
        NSInteger duration = [TIMER_SENDER integerValue];
        if(self.timeInteger) duration = self.timeInteger;
        [_skipBtn setTitle:[NSString stringWithFormat:@"%ld 跳过",(long)duration] forState:UIControlStateNormal];
...
- (void)dispath_tiemr{
    ...
    __block NSInteger duration = [TIMER_SENDER integerValue];
    if(self.timeInteger) duration = self.timeInteger;
...
- (void)animateEnd{
    CGFloat duration = [TIMER_SENDER floatValue];
...
+ (instancetype)initImageWithframe:(CGRect)frame imageURL:(NSString *)imageURL timeSecond:(NSInteger )timeSecond hideSkip:(BOOL)hideSkip LaunchAdCallback:(LaunchAdCallback)LaunchAdCallback launchAdClick:(LaunchAdClick)launchAdClick endPlays:(EndPlays)endPlays{
    ...
    ZJDLaunchAd *launchAd = [[ZJDLaunchAd alloc]initWithFrame:frame TimeInteger:timeSecond];
