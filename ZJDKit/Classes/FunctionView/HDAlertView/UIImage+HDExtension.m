...
+ (UIImage *)getImage:(UIImage *)sourceImage maxWidth:(CGFloat)maxWidth maxSize:(CGFloat) maxSize {
    ...
    // 默认 最大1MB
    if (maxSize <= 0.0) maxSize = 1000 * 1024.0;
    if (maxWidth <= 0.0) maxWidth = 1024.0;
    
    //先调整分辨率
    CGSize newSize = CGSizeMake(sourceImage.size.width, sourceImage.size.height);
    if (newSize.width > maxWidth) {
        newSize = CGSizeMake(maxWidth, maxWidth / sourceImage.size.width * sourceImage.size.height);
    }
    ...
    CGFloat sizeOriginKB = imageData.length;
    
    CGFloat resizeRate = 0.9;
    while (sizeOriginKB > maxSize && resizeRate > 0.1) {
        imageData = UIImageJPEGRepresentation(newImage,resizeRate);
        sizeOriginKB = imageData.length;
        resizeRate -= 0.1;
    }
    
    NSLog(@"图片大小：%f",sizeOriginKB);
