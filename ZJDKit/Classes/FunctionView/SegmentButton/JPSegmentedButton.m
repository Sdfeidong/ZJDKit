...
- (void)setSelectedIndex:(NSInteger)selectedIndex
{
    ...
    if (selectedIndex >= totalItems.count) {
        NSLog(@"ERROR:JPSegmentedButton无法设置越界下标“%ld”", (long)selectedIndex);
        return;
    }
...
- (void)insertOneSegment:(NSString *)title atIndex:(NSInteger)index
{
    ...
    if (index > totalItems.count) {
        NSLog(@"ERROR:JPSegmentedButton无法在越界下标“%ld”处添加按钮“%@”", (long)index, title);
        return;
    }
...
- (void)deleteOneSegmentAtIndex:(NSInteger)index
{
    // 判断是否越界
    if (index >= totalItems.count) {
        NSLog(@"ERROR:JPSegmentedButton无法在越界下标“%ld”处删除按钮", (long)index);
        return;
    }
